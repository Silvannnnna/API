...
import {getSalt, hashPassword} from "../uti/hash.js";
...
export const postUser = async (req, res) => {
    ...
    const sql = connectDB();
    const query = {
        text:"select * from users(name, username, password, points) values $1, $2, $3, $4, $5",
        values:[name, username, saltedHash, password, points],
    };
    const data = await sql.query(query);
    res.json(data.rows);
};
...
export const deleteUser = async (req, res) => {
    try {
      ...
      res.status(200).json({ msg: "se borro usuario" });
    } catch (error) {
